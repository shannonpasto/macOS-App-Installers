...
currentVers=$(/usr/bin/curl -s "${URL}" | /usr/bin/xmllint --html --xpath '//*[@id="home"]/div/div/div/div[1]/a[1]/text()' - 2>/dev/null | /usr/bin/awk '{print $2}' | /usr/bin/cut -c 2- - | xargs)
downloadURL=$(/usr/bin/curl -s "${URL}" | /usr/bin/xmllint --html --xpath '//*[@id="home"]/div/div/div/div[1]/a[1]/@href' - 2>/dev/null | /usr/bin/cut -d \" -f 2 -)
FILE=${downloadURL##*/}
