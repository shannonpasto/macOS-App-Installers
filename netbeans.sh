...
currentVers=$(/bin/echo "${latestReleaseTag}" | /usr/bin/sed 's/^v//' | /usr/bin/cut -d "-" -f -1 -)
downloadURL="${gitHubURL}/releases/download/${latestReleaseTag}/Apache-NetBeans-27-$(uname -m).pkg"
FILE=${downloadURL##*/}
