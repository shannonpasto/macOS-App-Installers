...
downloadURL="${gitHubURL}/releases/download/${latestReleaseTag}/XQuartz-${currentVers}.pkg"
SHAHash=$(/usr/bin/curl -Ls "${gitHubURL}/releases/download/${latestReleaseTag}/XQuartz-${currentVers}.pkg.sha256sum" | /usr/bin/awk '{print $2}')
FILE=${downloadURL##*/}
