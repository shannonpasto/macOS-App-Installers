...
if [ "$(/usr/bin/sw_vers -buildVersion | /usr/bin/cut -c 1-2 -)" -ge 24 ]; then
  ...
  currentVers=$(/usr/bin/curl -fs "https://product-details.mozilla.org/1.0/thunderbird_versions.json" | /usr/bin/plutil -extract LATEST_THUNDERBIRD_VERSION raw -o - -)
fi
