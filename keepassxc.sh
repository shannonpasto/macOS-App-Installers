...
downloadURL="${gitHubURL}/releases/download/${latestReleaseTag}/KeePassXC-${currentVers}-$(uname -m).dmg"
SHAHash=$(/usr/bin/curl -sL "${downloadURL}.DIGEST" | /usr/bin/awk '{print $1}')
FILE=${downloadURL##*/}
