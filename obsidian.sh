...
currentVers=$(/bin/echo "${latestReleaseURL}" | /usr/bin/rev | /usr/bin/awk -F v '{print $1}' | /usr/bin/rev)
downloadURL="${gitHubURL}/releases/download/${latestReleaseTag}/Obsidian-${currentVers}.dmg"
FILE=${downloadURL##*/}
